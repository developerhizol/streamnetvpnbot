...
class Database:
        ...
        with self._get_connection() as conn:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    free_from_no_sponsors INTEGER DEFAULT 0
                )
            """)
            ...
            try:
                conn.execute("ALTER TABLE users ADD COLUMN free_from_no_sponsors INTEGER DEFAULT 0")
            except sqlite3.OperationalError:
                pass
    ...
    def create_user(self, user_id: int, first_name: str, username: str = None):
        with self._get_connection() as conn:
            conn.execute("""
                INSERT INTO users (user_id, first_name, username, plan, free_until, subgram_linked, notify_24h_sent, notify_expired_sent, notify_1h_sent, free_from_no_sponsors)
                VALUES (?, ?, ?, 'free', NULL, 0, 0, 0, 0, 0)
            """, (user_id, first_name, username))
            conn.commit()
            logger.info(f"User created: {user_id} ({first_name}) with no active subscription")
        ...
        with self._get_connection() as conn:
            conn.execute("""
                UPDATE users SET is_premium = 1, premium_until = ?, plan = 'premium', 
                       free_until = NULL, free_from_no_sponsors = 0,
                       notify_24h_sent = 0, notify_expired_sent = 0, notify_1h_sent = 0 
                WHERE user_id = ?
            """, (premium_until, user_id))
            conn.commit()

    def extend_free_subscription(self, user_id: int, days: int = 3, from_no_sponsors: bool = False):
        """
        Выдаёт/продлевает бесплатную подписку.
        from_no_sponsors=True — подписка выдана из-за ошибки SubGram или отсутствия спонсоров.
                                 Если во время неё появятся спонсоры — она будет обнулена.
        from_no_sponsors=False — нормальная подписка после подписки на спонсоров.
                                  До конца срока не трогается, даже если появятся новые спонсоры.
        """
        moscow_now = get_moscow_time()
        current = self.get_free_until(user_id)
        base = current if current and current > moscow_now else moscow_now
        new_end = base + timedelta(days=days)
        
        with self._get_connection() as conn:
            conn.execute("""
                UPDATE users SET free_until = ?, free_from_no_sponsors = ?,
                       notify_24h_sent = 0, notify_expired_sent = 0, notify_1h_sent = 0 
                WHERE user_id = ?
            """, (new_end, 1 if from_no_sponsors else 0, user_id))
            conn.commit()
        return new_end

    def reset_free_subscription_if_no_sponsors(self, user_id: int) -> bool:
        """
        Обнуляет free_until, если подписка была выдана из-за отсутствия спонсоров,
        а сейчас спонсоры появились и есть неподписанные.
        Возвращает True, если подписка была сброшена.
        """
        with self._get_connection() as conn:
            row = conn.execute(
                "SELECT free_from_no_sponsors FROM users WHERE user_id = ?",
                (user_id,)
            ).fetchone()
            if row and row['free_from_no_sponsors'] == 1:
                conn.execute("""
                    UPDATE users SET free_until = NULL, free_from_no_sponsors = 0
                    WHERE user_id = ?
                """, (user_id,))
                conn.commit()
                logger.info(f"Free subscription reset for user {user_id} (sponsors appeared)")
                return True
        return False
