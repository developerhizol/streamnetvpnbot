...
class Database:
    ...
    def _init_db(self):
        with self._get_connection() as conn:
            ...
            # ВАЖНО: таблица admin_users удалена — данные админа хранятся только в .env
    ...
    # --- Проверка учётки админа теперь идёт через .env, а не через БД ---
    def check_admin_credentials(self, username: str, password: str) -> bool:
        if not ADMIN_USERNAME or not ADMIN_PASSWORD:
            logger.warning("ADMIN_USERNAME или ADMIN_PASSWORD не заданы в .env")
            return False
        return username == ADMIN_USERNAME and password == ADMIN_PASSWORD

    def get_admin_id(self, username: str) -> Optional[int]:
        # В БД больше нет таблицы admin_users, возвращаем фиксированный ID для логов
