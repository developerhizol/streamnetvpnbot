...
class Database:
    ...
    def create_user(self, user_id: int, first_name: str, username: str = None):
        # ИЗМЕНЕНО: при создании free_until = NULL.
        # Бесплатный доступ (3 дня) выдаётся только после подписки на спонсоров
        # через extend_free_subscription() в handle_check_subscription/handle_confirm_subscription.
        with self._get_connection() as conn:
            conn.execute("""
    ...
    def extend_free_subscription(self, user_id: int, days: int = 3):
        # ИЗМЕНЕНО: продление идёт от максимума (текущий free_until или now).
        # Если у пользователя уже была активная подписка — она не сбрасывается в 0,
        # а продлевается на days дней вперёд от текущей даты окончания.
        moscow_now = get_moscow_time()
