...
async def check_sponsors_periodically(bot: Bot):
    ...
    Логика:
    - Активные защищённые подписки (free_from_no_sponsors=0) не трогаем.
    - Если у пользователя «временная» подписка (free_from_no_sponsors=1) и появились
      новые спонсоры, на которых он не подписан — обнуляем подписку и уведомляем.
    - Если спонсоров нет / SubGram вернул ошибку и подписка истекла — продлеваем
      «временную» на 3 дня.
    - Если все спонсоры пройдены и подписка истекла — продлеваем «нормальную» на 3 дня.
    """
    while True:
        try:
            users = db.get_all_users()
            ...
            for user_id in users:
                try:
                    ...
                    # Пропускаем тех, кто ещё ни разу не взаимодействовал с SubGram
                    ...
                    # Активную защищённую подписку не трогаем до конца срока
                    if is_active and not free_from_no_sponsors:
                        continue

                    checked += 1

                    # Запрашиваем спонсоров у SubGram
                    try:
                    ...
                    # Нет спонсоров / ошибка SubGram → продлеваем «временную», если истекла
                    ...
                    # Есть новые спонсоры, на которых не подписан
                    if free_from_no_sponsors == 1:
                        # Временная подписка + появились спонсоры → обнуляем и уведомляем
