...
from config import BOT_TOKEN, PLATEGA_MERCHANT_ID, PLATEGA_SECRET, SUBGRAM_API_KEY, CRYPTOBOT_TOKEN
...
async def serve_home(request=None):
...
async def serve_sub_page(request=None):
    """Страница подписки (sub.html). Fallback — index.html."""
    ...
    return web.Response(text="sub.html / index.html not found", status=404)


async def serve_check(request=None):
...
async def handle_token_info(request):
    """
    GET /api/token/{token} — данные пользователя для веб-кабинета.
    Токен берём из path, если пусто — из query (?token=xxx).
    """
    token = request.match_info.get('token') or request.query.get('token')

    if not token or not token.strip():
        logger.warning(f"handle_token_info: empty token, path={request.path}, query={dict(request.query)}")
        return web.json_response({"error": "Token not provided"}, status=400)
    ...
    if not row:
        logger.warning(f"handle_token_info: token not found in DB: '{token}' (path={request.path})")
    ...
    logger.info(f"handle_token_info: OK user_id={user_id}, plan={plan}, active={is_active}")
...
async def start_webapp():
    ...
    # ===== API для веб-кабинета =====
    app.router.add_get('/api/token/{token}', handle_token_info)
    app.router.add_get('/api/user/{token}', handle_token_info)   # алиас
    app.router.add_get('/api/sub/{token}', handle_token_info)    # алиас
    app.router.add_get('/api/token', handle_token_info)          # query: ?token=xxx
