...
from aiogram.types import InlineKeyboardButton, BufferedInputFile, WebAppInfo
from aiogram.utils.keyboard import InlineKeyboardBuilder
from config import BOT_TOKEN, PLATEGA_MERCHANT_ID, PLATEGA_SECRET, SUBGRAM_API_KEY, SUBGRAM_API_URL, CRYPTOBOT_TOKEN
...
async def check_sponsors_periodically(bot: Bot):
    """
    Фоновая задача: раз в 30 минут проверяет спонсоров у free-пользователей.
    """
    while True:
        try:
            users = db.get_all_users()
            ...
            for user_id in users:
                try:
                    ...
                    if not user.get('subgram_linked'):
                        continue
                    ...
                    free_until = sub_info.get('free_until')
                    free_from_no_sponsors = sub_info.get('free_from_no_sponsors', 0)

                    is_active = False
                    if free_until:
                        if isinstance(free_until, str):
                            free_until = datetime.fromisoformat(free_until)
                        is_active = free_until > now

                    if is_active and not free_from_no_sponsors:
                        continue
                    ...
                    try:
                        ...
                        logger.error(f"SubGram error for user {user_id}: {e}")
                        if not is_active:
                            db.extend_free_subscription(user_id, days=3, from_no_sponsors=True)
                            extended_count += 1
                        continue

                    if sponsors_data is None or sponsors_data.get('status') == 'error':
                        if not is_active:
                            db.extend_free_subscription(user_id, days=3, from_no_sponsors=True)
                            extended_count += 1
                        continue
                    ...
                    if not need_subscribe:
                        if not is_active:
                            db.extend_free_subscription(user_id, days=3, from_no_sponsors=False)
                            extended_count += 1
                        continue

                    if free_from_no_sponsors == 1:
                        was_reset = db.reset_free_subscription_if_no_sponsors(user_id)
                        if was_reset:
                            reset_count += 1
                            try:
                                token = db.get_user_token(user_id)
                                ...
                                text = (
                                    f"{emoji('5447621159719827951', '🔔')} "
                                    f"<b>Появились новые спонсоры!</b>\n\n"
                                    f"Ваш доступ к VPN приостановлен. "
                                    f"Подпишитесь на спонсоров заново, чтобы продолжить пользоваться сервисом."
                                )
                                builder = InlineKeyboardBuilder()
                                builder.row(
                                    InlineKeyboardButton(
                                        text="Подписаться на спонсоров",
                                        web_app=WebAppInfo(
                                            url=f"https://{SUBSCRIPTION_DOMAIN}/sub/{token}"
                                        ),
                                ...
                                logger.error(f"Ошибка уведомления о появлении спонсоров для {user_id}: {e}")

                    await asyncio.sleep(0.05)
                except Exception as e:
                    logger.error(f"Ошибка проверки спонсоров для пользователя {user_id}: {e}")
                    continue

            logger.info(
                f"Фоновая проверка спонсоров завершена. "
                f"Проверено: {checked}, сброшено: {reset_count}, продлено: {extended_count}"
            )
            await asyncio.sleep(1800)
        except Exception as e:
            logger.error(f"Ошибка в check_sponsors_periodically: {e}")
            await asyncio.sleep(300)
...
async def serve_home():
    """Главная страница (index.html) — открывается по /"""
...
async def serve_sub_page():
    """Страница подписки (sub.html) — открывается браузером по /sub/{token}"""
    sub_path = Path(__file__).parent / 'public' / 'sub.html'
    if not sub_path.exists():
        return web.Response(text="sub.html not found", status=404)
...
async def handle_sub(request):
    """
    Логика для /sub/{token}:

    - Если это браузер:
        * plan != free  → sub.html
        * free-пользователь:
            - Активная защищённая подписка (уже получил 3 дня за спонсоров) → sub.html
            - Иначе проверяем спонсоров через SubGram:
                - Нет спонсоров / ошибка / не warning → sub.html
                - Есть спонсоры и есть те, на кого не подписан → check.html
                - Все спонсоры пройдены → sub.html
    - Если это приложение (Happ / V2RayTun и т.д.) → текст конфига
    """
    token = request.match_info.get('token', '')
    if not token:
    ...
    if is_browser(user_agent):
        ...
        # Premium — всегда страница подписки
        if plan != "free":
            return await serve_sub_page()

        # SubGram отключён — всегда страница подписки
        ...
        if free_until:
            if isinstance(free_until, str):
                free_until = datetime.fromisoformat(free_until)
            is_active = free_until > get_moscow_time()

        sub_info = db.get_subscription_info(user_id)
        free_from_no_sponsors = sub_info.get('free_from_no_sponsors', 0) if sub_info else 0

        # Активная "защищённая" подписка (получена за спонсоров) → sub.html
        # Пользователь уже подписался и получил свои 3 дня — страница проверки ему не нужна.
        if is_active and not free_from_no_sponsors:
            return await serve_sub_page()

        # Иначе — проверяем спонсоров
        try:
            subscriptions = await subgram.get_user_subscriptions(user_id)

            # Нет данных / ошибка → sub.html (пусть подписка выдастся или обновится)
            ...
            # Есть спонсоры, на которых не подписан → check.html
            if need_subscribe:
                # Сбрасываем "временную" подписку, если она была
                db.reset_free_subscription_if_no_sponsors(user_id)
...
async def handle_check_subscription(request):
    ...
    try:
        ...
        if sponsors_data is None or sponsors_data.get('status') == 'error':
            new_free_until = db.extend_free_subscription(user_id, days=3, from_no_sponsors=True)
        ...
        if not need_subscribe:
            new_free_until = db.extend_free_subscription(user_id, days=3, from_no_sponsors=False)
            db.update_subgram_linked(user_id, 1)
            return web.json_response({
                "status": "ok",
...
async def handle_confirm_subscription(request):
    ...
    try:
        ...
        if subscriptions is None or subscriptions.get('status') == 'error' or subscriptions.get('status') != 'ok':
            new_free_until = db.extend_free_subscription(user_id, days=3, from_no_sponsors=True)
            db.update_subgram_linked(user_id, 1)
            return web.json_response({
                "success": True,
        ...
        for webhook in webhooks:
            ...
            if status == 'unsubscribed':
                ...
                with db._get_connection() as conn:
                    conn.execute("""
                        UPDATE users SET free_until = ?, free_from_no_sponsors = 0 WHERE user_id = ?
...
async def handle_cryptobot_webhook(request):
    """Вебхук от CryptoBot. Проверяет подпись HMAC-SHA256."""
    try:
        ...
        logger.info(f"CryptoBot invoice paid: id={invoice_id}, payload={payload_str}, amount={amount}")
        ...
        if len(parts) != 3:
            logger.error(f"CryptoBot webhook: invalid payload format: {payload_str}")
            return web.Response(status=200, text="OK")
        ...
        if not plan:
            logger.error(f"CryptoBot webhook: unknown plan_key={plan_key}")
            return web.Response(status=200, text="OK")
        ...
        try:
            ...
            logger.error(f"Error sending CryptoBot payment notification to user {user_id}: {e}")
...
async def start_webapp():
    ...
    app.router.add_get('/', serve_home)          # ← главная страница
...
def main():
    ...
    async def run_all():
        ...
        asyncio.create_task(check_sponsors_periodically(bot))
